...
spearmanr_GNNExpl_top = [] 
pearsonr_GNNExpl_top = [] 
kendalltau_GNNExpl_top = [] 

spearmanr_tree_explainer = [] 
pearsonr_tree_explainer = [] 
kendalltau_tree_explainer= []

spearmanr_tree_explainer_top = [] 
pearsonr_tree_explainer_top = [] 
kendalltau_tree_explainer_top = [] 
...
for test_set_index in tqdm(test_compounds_indices):
    ...
    if SAVE:
        if TRAINING_SET_SPLIT == None:
            img.save(CPD_FOLDER_NAME + "/" + "EdgeSHAPer_MC_" + str(M) + "_heatmap.png")
        else:
            img.save(CPD_FOLDER_NAME + "/" + "EdgeSHAPer_MC_" + str(M) + "_train_" + str(TRAINING_SET_SPLIT) + "_heatmap.png") 
    ...
    if GNNEXPLAINER_USAGE:
        ...
        if COMPUTE_CORRELATION:
            num_top_bonds = int(round(0.25*len(rdkit_bonds_phi)))

            important_bonds_ranking_edgeshaper = np.argsort(-np.array(rdkit_bonds_phi))

            ###between EdgeSHAPer and GNNExplainer #####
            if GNNEXPLAINER_USAGE:
                important_bonds_ranking_GNNExpl = np.argsort(-np.array(rdkit_bonds_GNNExpl_importance))
                
                spearmanr_GNNExpl_current = spearmanr(important_bonds_ranking_edgeshaper, important_bonds_ranking_GNNExpl)[0]
                pearsonr_GNNExpl_current = pearsonr(important_bonds_ranking_edgeshaper, important_bonds_ranking_GNNExpl)[0]
                kendalltau_GNNExpl_current = kendalltau(important_bonds_ranking_edgeshaper, important_bonds_ranking_GNNExpl)[0]

                spearmanr_GNNExpl_top_current = spearmanr(important_bonds_ranking_edgeshaper[:num_top_bonds], important_bonds_ranking_GNNExpl[:num_top_bonds])[0]
                pearsonr_GNNExpl_top_current = pearsonr(important_bonds_ranking_edgeshaper[:num_top_bonds], important_bonds_ranking_GNNExpl[:num_top_bonds])[0]
                kendalltau_GNNExpl_top_current = kendalltau(important_bonds_ranking_edgeshaper[:num_top_bonds], important_bonds_ranking_GNNExpl[:num_top_bonds])[0]

                spearmanr_GNNExpl.append(spearmanr_GNNExpl_current)
                pearsonr_GNNExpl.append(pearsonr_GNNExpl_current)
                kendalltau_GNNExpl.append(kendalltau_GNNExpl_current)

                spearmanr_GNNExpl_top.append(spearmanr_GNNExpl_top_current)
                pearsonr_GNNExpl_top.append(pearsonr_GNNExpl_top_current)
                kendalltau_GNNExpl_top.append(kendalltau_GNNExpl_top_current)

            ###between EdgeSHAPer and TreeExplainer #####
            cpd_bond_dict = pickle.load(open("../data/bond_weights_tree_explainer/bond_weights.pkl", "rb"))
            cpd_bond_weights = cpd_bond_dict[test_cpd.smiles]["bond_SVs"]

            important_bonds_ranking_tree_explainer = np.argsort(-np.array(list(cpd_bond_weights.values()))) #the order is coherent with rdkit_bonds_phi and rdkit_bonds_GNNExpl_importance

            spearmanr_tree_explainer_current = spearmanr(important_bonds_ranking_edgeshaper, important_bonds_ranking_tree_explainer)[0]
            pearsonr_tree_explainer_current = pearsonr(important_bonds_ranking_edgeshaper, important_bonds_ranking_tree_explainer)[0]
            kendalltau_tree_explainer_current = kendalltau(important_bonds_ranking_edgeshaper, important_bonds_ranking_tree_explainer)[0]

            spearmanr_tree_explainer_top_current = spearmanr(important_bonds_ranking_edgeshaper[:num_top_bonds], important_bonds_ranking_tree_explainer[:num_top_bonds])[0]
            pearsonr_tree_explainer_top_current = pearsonr(important_bonds_ranking_edgeshaper[:num_top_bonds], important_bonds_ranking_tree_explainer[:num_top_bonds])[0]
            kendalltau_tree_explainer_top_current = kendalltau(important_bonds_ranking_edgeshaper[:num_top_bonds], important_bonds_ranking_tree_explainer[:num_top_bonds])[0]

            spearmanr_tree_explainer.append(spearmanr_tree_explainer_current)
            pearsonr_tree_explainer.append(pearsonr_tree_explainer_current)
            kendalltau_tree_explainer.append(kendalltau_tree_explainer_current)

            spearmanr_tree_explainer_top.append(spearmanr_tree_explainer_top_current)
            pearsonr_tree_explainer_top.append(pearsonr_tree_explainer_top_current)
            kendalltau_tree_explainer_top.append(kendalltau_tree_explainer_top_current)
...
if COMPUTE_CORRELATION:
    print("\n=========================================================")
    print("\nRank correlation coefficients between bond rankings: EdgeSHAPer vs TreeExplainer")

    print("Full ranking:")
    print("Spearman: ",  round(sum(spearmanr_tree_explainer)/len(spearmanr_tree_explainer), 3))
    print("Pearson: ",  round(sum(pearsonr_tree_explainer)/len(pearsonr_tree_explainer), 3))
    print("Kendall tau: ",  round(sum(kendalltau_tree_explainer)/len(kendalltau_tree_explainer), 3))

    print("Top 25'%' of bonds:")
    print("Spearman: ",  round(sum(spearmanr_tree_explainer_top)/len(spearmanr_tree_explainer_top), 3))
    print("Pearson: ",  round(sum(pearsonr_tree_explainer_top)/len(pearsonr_tree_explainer_top), 3))
    print("Kendall tau: ",  round(sum(kendalltau_tree_explainer_top)/len(kendalltau_tree_explainer_top), 3))

    if GNNEXPLAINER_USAGE:
        print("\nRank correlation coefficients between bond rankings: EdgeSHAPer vs GNNExplainer")

        print("Full ranking:")
        print("Spearman: ",  round(sum(spearmanr_GNNExpl)/len(spearmanr_GNNExpl), 3))
        print("Pearson: ",  round(sum(pearsonr_GNNExpl)/len(pearsonr_GNNExpl), 3))
        print("Kendall tau: ",  round(sum(kendalltau_GNNExpl)/len(kendalltau_GNNExpl), 3))

        print("Top 25'%' of bonds:")
        print("Spearman: ",  round(sum(spearmanr_GNNExpl_top)/len(spearmanr_GNNExpl_top), 3))
        print("Pearson: ",  round(sum(pearsonr_GNNExpl_top)/len(pearsonr_GNNExpl_top), 3))
        print("Kendall tau: ",  round(sum(kendalltau_GNNExpl_top)/len(kendalltau_GNNExpl_top), 3))